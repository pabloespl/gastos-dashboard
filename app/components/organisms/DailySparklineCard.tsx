...
export function DailySparklineCard({ summary }: Props) {
  ...
  const last25   = dailyTotals.slice(-25)
      ...
      {last25.length === 0 ? (
        ...
        <>
          <div className="flex h-16 items-end gap-0.5">
            {last25.map(({ date, total }) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-full rounded-sm ${isToday ? 'bg-primary-bar' : 'bg-border group-hover:bg-border-strong'}`}
          ...
          <div className="mt-4 flex justify-between text-xs">
            <div>
              <p className="text-text-muted">Día más alto</p>
              {highDay ? (
                <p className="font-medium text-text-primary">
                  {formatCLP(highDay.total)}{' '}
                  <span className="font-normal text-text-muted">· {highDay.date.slice(8)}</span>
                </p>
              ) : (
            ...
            <div className="text-right">
              <p className="text-text-muted">Proyección del mes</p>
              <p className="font-medium text-text-primary tabular-nums">{formatCLP(projection)}</p>
            </div>
          </div>
        </>
      )}
